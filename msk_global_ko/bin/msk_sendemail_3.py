#Python 3 version with minor modifications around line 688 to handle HTML emails. The core splunk version didnt have this build in. This script is mostly an exact copy from Splunk's own send email script with 1 change.
import copy
import sys
...
def buildHTMLBody(ssContent, results, settings, emailbody, email, jobCount):
    ...
    messageHTML = ssContent.get('action.email.message')												   
    resultsHTML = ''
    metaDataHTML = ''
    errorHTML = ''
